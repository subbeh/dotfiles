...
return {
  ...
  on_init = function(client)
    if client.workspace_folders then
      ...
      if path ~= vim.fn.stdpath("config") and path ~= nvim_config and vim.fs.normalize(vim.fs.dirname(path)) ~= plug_dir and (vim.uv.fs_stat(path .. "/.luarc.json") or vim.uv.fs_stat(path .. "/.luarc.jsonc")) then
        return
      end
    end
